...
class CrossValidation:
    ...
    def CrossValidationPerson(self, PARAM_C, PARAM_Gamma, NUM_TRIALS, KSPLITS):
        ...
        for j in range(0,NUM_TRIALS):
            ...
            #todo: continue here: docela tam litaji indexy mezi vsim a vsema.
            # zkontrolat - co za skupinu se zpracovava
            # zkontrolovat - jake hodnoty na jakych indexes tam zrovna jsou
            # zkontrolovat jaky typ se ma zrovna kam predavat: ndarray x pd.dataframe
            # jinak se stane, ze se budeme strilet do nohy a budem si tam posilat ruzne loc a iloc data
    ...
    def CrossvalidationUsersLongterm(self, bestParams, testTag):
            ...
            #todo: pokracujeme tady - potrebujeme mit stejne rozmery bud array of tupples nebo neco jineho
            # # auc = roc_auc_score(dfTestProbabilities, dfTestProbabilities)
            # dfTestProbabilities.columns = ['label', '0', '1', 'user']
            #
            # # y_test = [tuple(x) for x in dfTestProbabilities.iloc[:,0].values] #todo: continue here - potrebujeme list of tuples
            # # y_scores = [tuple(x) for x in dfTestProbabilities.iloc[:,1:3].values]
            #
            # auc_positive = roc_auc_score(dfTestProbabilities.iloc[:,0], dfTestProbabilities.iloc[:,1])
            # auc_negative = roc_auc_score(dfTestProbabilities.iloc[:,0], dfTestProbabilities.iloc[:,2])
