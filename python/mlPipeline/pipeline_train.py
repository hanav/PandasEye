...
class ParameterSearch:
    ...
    def GridSearch(self):
            ...
            #self.cv  =LabelKFold(self.y_train,n_folds=2) #todo nefunguje
            ...
            #self.best_C = gridSearch.best_params_['C']
            #self.best_gamma = gridSearch.best_params_['gamma']
            #self.best_kernel = gridSearch.best_params_['kernel']

            # self.gridDf = pd.DataFrame(data=None)
            # for params, mean_score, scores in gridSearch.grid_scores_:
            #     row = pd.DataFrame(data=[mean_score, scores.std()*2, params]).transpose()
            #     self.gridDf = self.gridDf.append(row)
            #
            # self.gridDf #output accuracies and parameters
    ...
    def SimpleTrain(self,PARAM_C, PARAM_Gamma): #stand-alone command
        ...
        #todo: add also number of features
        ...
        #confusionMatrix_stats_train = classification_report(y_train, train_predict, target_names=target_names)
        #confusionMatrix_stats_test = classification_report(y_test, test_predict, target_names=target_names)
