...
class EyeFrame:
    ...
    def countLineDistance(self):
        ...
        # gazeYpairs = zip(self.cleanData.gazey, self.cleanData.gazey[1:])
        # uniqueGazeYpairs = np.unique(gazeYpairs)
        # gazeDiffs = uniqueGazeYpairs[:,1] - uniqueGazeYpairs[:,0]
        # gazeDiffs

        #gazeYlines = self.cleanData.gazey[np.where( self.cleanData.linenum == np.unique(linenums))]
        #gazeYlines #todo: pokracovani tady, nefunguje porovnavani subsets

    def countMeanTransitionDistnace(self):
        # in line numbers, later todo: in pixels
