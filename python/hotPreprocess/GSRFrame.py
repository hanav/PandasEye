...
class GSRFrame:
    ...
    def preprocessing(self, baseline): # todo: GSR baseline - remove
        ...
        #series = series - series[0] # debug id: 13, for after comment - empty frame - dohledat proc6

        # normalize it - z-score is also fine
    ...
    def countTonicFeatures(self): # propagate here the baseline value from the first 10s of recordings
        ...
        PCPS = ((self.tonic - self.tonic.mean())/self.tonic.std())/self.tonic.mean() #todo: this should be done from the whole recording
    ...
    def countPhasicFeatures(self):
        ...
        spacing = (self.phasic.index[1] - self.phasic.index[0]).total_seconds() # turn this into ms
        ## Get Power Spectral Density
        signalPSD = np.abs(ff_phasic) ** 2
        fftFreq = fftfreq(len(signalPSD), spacing)
        #plt.plot(fftFreq[i], 10 * np.log10(signalPSD[i]));
        hist = np.histogram(signalPSD, bins = fftFreq[fftFreq > 0]) # todo: too many zeros, continue here

    def countPhasicPeaks(self):
        pass
