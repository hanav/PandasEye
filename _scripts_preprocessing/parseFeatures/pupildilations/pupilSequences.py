...
# MFCC - http://stackoverflow.com/questions/5835568/how-to-get-mfcc-from-an-fft-on-a-signal
...
class PupilSequences:
    def __init__(self):
        ...
        self.fixStarts = [0,18,37,50] #zmenit: druhy for pres vsechny lidi
        self.kernel = np.array([1,1,1], dtype=np.float64)
        self.kernel = self.kernel/3

    def load(self, data): 
        self.data = data
        self.fixationCount = 3 #todo: toto zmenit na opravdovy pocet fixaci
    ...
    def extractPupils(self):
        ...
        for seq in self.data:
            ...
            #self.fixStarts.append(starts)

        # plt.axvline(x=len(self.pupils)/2, color='r')
    ...
    def normalizeEventPCPS(self,seq):
        mean = np.mean(seq)
        std = np.var(seq)
        result = ((seq - mean)/std)/mean #todo: tady to mame blbe
        return result

    def countPCPS(self, seq): #todo je opravena PCPS
        mean = np.mean(seq)
