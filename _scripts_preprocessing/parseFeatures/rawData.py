...
class RawData:
    ...
    def findNearestFixationBeforeEvent(self):
        ...
        for idx in self.eventDf.index:
            ...
            try:
                ...
                #print "Inf inside:", fixations
                continue

        #self.eventDf = self.eventDf.replace([np.inf, -np.inf], np.nan)
    ...
    #todo: umazat prvnich pet minut nebo lepe nadetekovat, kde zacal exp.
    def separateEvents8P(self):
        return self.eventDf.loc[self.eventDf['Event']=='LMouseButton']

#vypiplame pryc eventy, nenesou zadnou eye-movement hodnotu
